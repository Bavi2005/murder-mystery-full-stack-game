...
function getKey(): Buffer {
  ...
  const secret = config.encryption.key || '32_char_encryption_key_change_me';
  return crypto.createHash('sha256').update(secret).digest();
}

export function encryptEnvelope<T>(plain: T): string {
...
export function decryptEnvelope<T>(payload: string): T {
  const [ivB64, tagB64, dataB64] = payload.split('.');
  if (!ivB64 || !tagB64 || !dataB64) throw new Error('Invalid envelope payload');
